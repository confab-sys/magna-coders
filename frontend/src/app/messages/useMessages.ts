...
export function useMessages() {
          ...
          messages: [
            {
              ...
              senderId: 'Me',
              content: 'I have accepted your job application. Let\'s discuss the details.',
              createdAt: new Date().toISOString(),
              type: 'TEXT',
              isRead: true,
              sender: {
      ...
      try {
        ...
        const mappedConversations: Conversation[] = chats.map((chat: ApiChat) => {
            ...
            return {
              ...
              type: chat.type,
              name: name,
              participants: [], // In real app, map participants
          ...
          if (c.id === selectedChatId) {
            return {
              ...
              messages: messages.map((m: ApiMessage) => ({
                ...
                sender: {
                  id: m.senderId,
                  username: m.senderId === currentUserId ? 'Me' : 'Other',
                  fullName: m.senderId === currentUserId ? 'Me' : 'Other'
                }
              }))
            };
          }
  ...
  const filteredConversations = conversations.filter(c => {
    ...
    if (searchQuery) {
      ...
      // Ensure lastMessage is treated as a string for inclusion check
      const lastMsgContent = typeof c.lastMessage === 'string' ? c.lastMessage : c.lastMessage?.content || '';
      return (
        (c.name || '').toLowerCase().includes(query) || 
        lastMsgContent.toLowerCase().includes(query)
      );
    }
    ...
    if (USE_REAL_API) {
      try {
        ...
        const newGroup: Conversation = {
          ...
          participants: [], // Real app should map this
